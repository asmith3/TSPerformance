  ...
  output$chart <- renderChart({
    ...
    if(input$radio == "Yields") {
      if(input$time == "5 Years"){subdata <- tspdata[,18:22][seq(1, nrow(tspdata), by=21),]; 
                                  ...
                                  c <- Highcharts$new()
                                  c$chart(type = "spline", height="450", width="800", zoomType="x")
                                  c$title(text = paste("TSP ", input$radio, ", Past ", input$time))
                                  c$set(dom = "chart")
                                  
                                  c$xAxis(title = list(text = "Month"), categories = labels
                                          , plotLines = (list(list(value=12, color='#B7CEEC', width=.75, dashStyle='longdash'),
                                                              list(value=24, color='#B7CEEC', width=.75, dashStyle='longdash'),
                                                              list(value=36, color='#B7CEEC', width=.75, dashStyle='longdash'),
                                                              list(value=48, color='#B7CEEC', width=.75, dashStyle='longdash')
                                          )))
